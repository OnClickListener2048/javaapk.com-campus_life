...
public class PersonnalActivity extends Activity implements OnClickListener {
    ...
    protected void onResume() {
        super.onResume();
        int identity = SPUtils.getInstance().getInt("identity");
        if (identity == Constants.INDIVIDUAL) {
            publish.setVisibility(View.GONE);
            look.setVisibility(View.VISIBLE);
        }

        if (identity==Constants.GROUP) {
	...
	public void onClick(View v) {
		...
		case id.message:
			intent = new Intent(PersonnalActivity.this,
					MyMsgActivity.class);
			startActivity(intent); //我的留言
			break;
		case id.collection:
			intent = new Intent(PersonnalActivity.this,
					MyCollectionActivity.class);//我的收藏
			startActivity(intent); 
			break;
		case id.look:
			intent = new Intent(PersonnalActivity.this,
					MyLookActivity.class);//我的求职信息
			startActivity(intent); 
			break;
		default:
			break;
